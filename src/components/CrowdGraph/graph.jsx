...
class CrowdGraph extends Component {
  ...
  componentDidMount() {
    ...
    if (tfcunitId){
      this.setState({
        ...
        this.getcrowdTripMode(); //获取人群出行方式柱状图信息接口
        this.getTrafficDistanceCompare(); //当月和上月人员出行距离对比
        this.getTripTimeHold(); //出行时间占比（人群分析）
      })
    }
  }
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (isTripMode||isStatMonth||isTravalPeriod||isAreaType||isTfcunitId||isOdTfcunitId) {
      this.setState({
        ...
        this.getTripTimeHold(); //出行时间占比（车辆分析
      })
    }
  }
      ...
      trafficDistanceCompare({
        ...
        if (res) {
          if (res.code === 200) {
            ...
            let curData = [0, 0, 0, 0, 0]; //当月人群出行距离占比
            let lastData = [0, 0, 0, 0, 0]; //上月人群出行距离占比
  ...
  //出行时间占比（人群分析）  0-10 10-20  20-30  30-40 >40
  getTripTimeHold = () => {
    const {echartDataId, statMonth, echartOdTfcunitId, trlTypeNo} = this.state;
    this.setState({
      tripTimeLoding:true
      ...
      tripTimeHold({
        ...
        if (res) {
          if (res.code === 200) {
            ...
            let curTimeData = [0, 0, 0, 0, 0]; //当月人群出行时间占比
            let lastTimeData = [0, 0, 0, 0, 0]; //上月人群出行时间占比
            res.data.cur.forEach(item => {
              xAxis.forEach((axis, index) => {
