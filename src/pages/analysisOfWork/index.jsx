...
class AnalysisOfWork extends Component {
  ...
  state = {
    ...
    tripPageNum: 1, //出行特征的当前页
    tripPageSize: 10, //出行特征的数据大小
    flowPageTotal: 0, //总页数
    liveDensity: '', //居住地人口密度
    workDensity: '', //工作地人口密度
  ...
  getTripFeatureAnalyze = () => {
    ...
    connector({
      ...
      pageSize: 10,
      trlTypeNo: fieldsValue.tripMode, //出行方式 -1全量出行 1,2公交轨交 3出租网约 4私家车
      ...
      if (res) {
        ...
        if (res.code === 200) {
          ...
          if (portFlag === 1) {
            ...
            createAnalysisData = utils.inOriginDataAddCenterArray(res.data.records, 'centerCoor', 'odCenterCoor', 'avgTrlTime', portFlag); //按出行距离排序
          } else {
            createAnalysisData = utils.inOriginDataAddCenterArray(res.data.records, 'centerCoor', 'odCenterCoor', 'avgTrlDistance', portFlag); //按出行距离排序
          }
          this.setState({
  ...
  renderMapArea = (data) => {
    ...
    data.map(item => {
      ...
      const polygon = L.polygon(boundPolys, {
        ...
        click: e => {
          let layer = e.target;
          const { selectFlag, tfcunitIds } = this.state;
    ...
    return (
      <div className={styles.wrap_index}>
        ...
        <Spin size="large" spinning={(mapAreaLoading || flowLoading)}>
                          ...
                          renderItem={(item, index) => (
                            ...
                            >
                              <List.Item.Meta
                                ...
                                description={portFlag === 1 ? `出行客流量：${item.addSortVariableText}` : portFlag === 2 ? `出行时间：${item.addSortVariableText}` : `出行距离${item.addSortVariableText}`}
                              />
                            </List.Item>
                          )}
                        />
          ...
          {/*  右边的卡片---人群出行分析和交通出行分析 isShowRight showEchart*/}
          <div
            className={
              isShowRight
                ? styles.rightCard
